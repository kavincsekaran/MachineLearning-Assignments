...
def gaussian_kernel(X1, X2,gamma=1.):
    ...
    for x1 in X1:
        j=0
        for x2 in X2:
            K[i,j] = np.exp(-1*np.linalg.norm(x1-x2)**2/(2.*gamma**2))
            j+=1
        i+=1
...
def predict(K, a, y, b):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    y_test=np.dot(np.multiply(a,y).T,K.T)+b
    constrained_y=[]
    for y_i in np.asarray(y_test)[0]:
        if(y_i>0):
            constrained_y.append([1])
        else:
            constrained_y.append([-1])
    #########################################
    return np.matrix(constrained_y)
...
def compute_E(Ki,a,y,b,i):
    ...
    E=np.dot(np.multiply(a,y).T,Ki.T)+b-y[i]
    E=float(np.asarray(E)[0][0])
