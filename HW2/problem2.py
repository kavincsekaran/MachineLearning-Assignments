...
def compute_a(z):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    e_z=np.exp(np.float128(-z))
    
    #e_z = np.clip(e_z, sys.float_info.min,sys.float_info.min)
    a = float(1. / (1. + e_z))
...
def compute_L(a,y):
    ...
    ## INSERT YOUR CODE HERE
    sigma=1e-10
    if((a==1 and y ==0) or (a==0 and y==1)):
        L=sys.float_info.max
    else:    
        L=np.float(-y*np.log(a+sigma)-(1-y)*np.log((1-a)+sigma))
...
def compute_dL_da(a, y):
    ...
    if y == 0:
        if a != 1:
            ...
            dL_da = sys.float_info.max
    else:
        if a != 0:
            dL_da = -1 / a
        else:
            dL_da = sys.float_info.min
