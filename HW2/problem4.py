...
def compute_a1(z1):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    a1 = np.mat(np.zeros((len(z1),1)))
    for i in range(len(z1)):
        a1[i] = float(1 / (1 + np.exp(np.float128(-z1[i]))))
    #########################################
    return a1 
...
def compute_dL_da2(a2, y):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    dL_da2 = np.mat(np.zeros((len(a2),1)))
    if a2[y] != 0:
        dL_da2[y] = -1 / a2[y]
    else:
        dL_da2[y] = -sys.float_info.max
    #########################################
    return dL_da2
...
def compute_dz2_dW2(a1,c):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    dz2_dW2 = np.mat(np.zeros((c,len(a1))))
    for i in range(c):
...
def train(X, Y,h=3, n_layers=3, alpha=0.01, n_epoch=100):
    ...
    for _ in xrange(n_epoch):
        ...
        for x,y in zip(X,Y):
            ...
            # Forward pass
            z1, a1, z2, a2 = forward(x, W1, b2, W2, b2)
