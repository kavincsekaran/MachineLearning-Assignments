...
class Tree(object):
    ...
    def entropy(Y):
        ...
        if n_classes > 1:
            e=np.sum(-np.multiply(probs,np.log2(probs)))
    ...
    def conditional_entropy(Y,X):
        ...
        count_X = dict(zip(value,counts))
        total_count = len(Y)
        ent = 0
        for x, x_count in count_X.items():
            ent += x_count * Tree.entropy(Y[X == x])
        ce = ent / float(total_count)
    ...
    def best_attribute(X,Y):
        ...
        ig=[]
        for x in X:
            ig.append(Tree.information_gain(Y,x))
        i = np.argmax(ig)
    ...
    def split(X,Y,i):
        ...
        C={}
        i_col=X[i,:]
        i_values=np.unique(i_col)
        for ind_i in i_values:
            #indices=np.equal(i_col, ind_i)
            indices=i_col==ind_i
            C[ind_i]=Node(X[:,indices],Y[indices])
    ...
    def build_tree(t):
        ...
        if(Tree.stop1(t.Y) or Tree.stop2(t.X)):
            ...
            for split, child in t.C.iteritems():
    ...
    def inference(t,x):
        ...
        #########################################
        ## INSERT YOUR CODE HERE
        if(t.C==None or t.isleaf):
            y=t.p
        elif(x[t.i] not in t.C.keys()):
            y=t.p
        else:
            y=Tree.inference(t.C.get(x[t.i]), x)
    ...
    def load_dataset(filename='data1.csv'):
        ...
        ## INSERT YOUR CODE HERE
        file_data=[]
        with open(filename, 'r') as ds_file:
            next(ds_file)
            for line in ds_file:
