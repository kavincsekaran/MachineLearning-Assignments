...
class LSTM(sr):
    ...
    def gates(self, x, H):
        ...
        #########################################
        ## INSERT YOUR CODE HERE
        h_x=th.cat((x, H), dim=1)
        
        f=th.sigmoid(th.matmul(h_x, self.W_f)+self.b_f)
        i=th.sigmoid(th.mm(h_x, self.W_i)+self.b_i)
        o=th.sigmoid(th.mm(h_x, self.W_o)+self.b_o)
        C_c=tanh(th.mm(h_x, self.W_c)+self.b_c)
        
        #f=th.sigmoid(th.mm(h_x, self.W_f)+self.b_f.expand(self.W_f.shape[1], self.b_f.shape[0]))
        #i=th.sigmoid(th.mm(h_x, self.W_i)+self.b_i.expand(self.W_i.shape[1], self.b_i.shape[0]))
        #o=th.sigmoid(th.mm(h_x, self.W_o)+self.b_o.expand(self.W_o.shape[1], self.b_o.shape[0]))
        #C_c=tanh(th.mm(h_x, self.W_c)+self.b_c.expand(int(self.W_c.shape[1]), self.b_c.shape[0]))
    ...
    def forward(self, x, H,C):
        ...
        C_new=self.update_cell(C, C_c, i)

        H_new=self.output_hidden_state(C, o)
        
        z = th.matmul(H_new, self.W) +self.b
