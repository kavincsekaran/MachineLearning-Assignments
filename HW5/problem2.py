...
class CBandit:
    ...
    def step(self, a):
        ...
        r=np.random.choice((0,1), p=[1-self.p[self.s, a], self.p[self.s, a]])
        s = np.random.choice(np.arange(0, self.n_s, 1))
        self.s=s
...
class Agent(object):
   ...
   # ----------------------------------------------
    def forward(self,s):
        ...
        if(rand_choice):
            a=np.random.choice(self.n, p=[(1./self.n)]*self.n)
        else:
    ...
    def update(self,s,a,r):
        ...
        wins=np.multiply(self.c, self.Q)
        wins[s, a]+=r
        self.c[s, a]+=1
        self.Q=np.divide(wins, self.c, out=np.zeros_like(wins), where=self.c!=0)   
