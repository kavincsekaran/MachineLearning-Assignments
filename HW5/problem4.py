...
class QNet(object):
    ...
    def forward(self, s):
        ...
        rand_choice=np.random.choice((True,False), p=[self.e,(1-self.e)])
        
        if(rand_choice):
            a=np.random.choice(self.n)
        else:
            #Q=self.compute_Q(s).data.numpy()
            #a=np.random.choice(np.flatnonzero(Q==Q.max()))
            #print(np.flatnonzero(Q==Q.max()))
            _, a = self.compute_Q(s).max(dim=0)
    ...
    def compute_L(self,s,a,r,s_new, gamma=.95):
        ...
        # compute target Q
        part_Q, _=self.compute_Q(s_new).max(dim=0)
        target_Q=Variable(th.add(th.mul(part_Q, gamma), r).data, requires_grad=False)
        
        # get current Q
        Q=self.compute_Q(s)[a]
        
        # compute loss
        L=th.nn.functional.mse_loss(Q, target_Q, size_average=False)
